...
def command_add(date, start_time, end_time, title, calendar):
    ...
    if start_time <= end_time and start_time > 0 and end_time > 0 and start_time < 24 and end_time < 24:
        if date not in calendar:
            ...
            calendar[date].sort(key=lambda start: start['start'])
        return True
    else:
        return False
...
def command_show(calendar):
    ...
    calendar_string = ""
    for date in list(reversed(sorted(calendar))):
        calendar_string += "\n" + date + " : "
        for x in calendar[date]:
            start = ('%02d' % (x['start']) + ':00')
            end = ('%02d' % (x['end']) + ':00')
            title = x["title"]
            calendar_string += "\n    start : " + start + ",\n    end : " + end + ",\n" + "    title : " + title
            if len(calendar[date]) > 1 and x != calendar[date][-1]:
                calendar_string += "\n"
...
def command_delete(date, start_time, calendar):
    ...
    if not is_calendar_date(date):
        ...
        if date not in calendar:
            ...
            for start in calendar[date]:
                if start_time == start['start']:
                    calendar[date].remove(start)
...
def save_calendar(calendar):
    ...
    for date in list(reversed(sorted(calendar))):
        calendar_string = ""
        for x in calendar[date]:
            calendar_string += str(('%02d' % (x['start']))) + "-" + str(('%02d' % (x['end']))) + " " + x['title'] + '\t'
        saved += date + ":" + calendar_string.strip("\t.") + "\n"
...
def load_calendar():
    ...
    if not file.read(1):
        ...
        file.seek(0)
        days = file.read().strip().split("\n")
        for x in days:
            date = x.split(":")[0]
            tasks = x.split(":")[1].split("\t")
            calendar[date] = []
            for y in tasks:
                time_title = y.split()
                time = time_title.pop(0)
                title = ""
                for z in time_title:
                    title += z + " "
                calendar[date].append({"start": int(time.split("-")[0]), "end": int(time.split("-")[1]), "title": title.rstrip()})
                file.close()
    return calendar
# -----------------------------------------------------------------------------
...
def is_command(command):
    ...
    # YOUR CODE GOES HERE
    if command.lower() == "add" or command == "delete" or command == "quit" or command == "help" or command == "show":
        return (True)
    else:
        return (False)
...
def is_calendar_date(date):
    ...
    if not is_natural_number(year) or not is_natural_number(month) or not is_natural_number(day):
        return False
...
def parse_command(line):
    ...
    if len(line) == 0 or line == "help":
        ...
        answer = line.split(" ")
        if answer[0].lower() == "add":
            if len(answer) >=1 and len(answer) <=3:
                answer = ['error', 'add DATE START_TIME END_TIME DETAILS']
            if len(answer) == 4:
                if not is_natural_number(answer[2]):
                    del answer
                    answer = ["help"]
                else:
                    answer = ['error', 'add DATE START_TIME END_TIME DETAILS']
            if len(answer) > 4:
                if is_calendar_date(answer[1]):
                    title_list = []
                    for x in range(4, len(answer)):
                        title_list.append(answer[x]+ ' ')
                    title = ''.join(title_list)
                    answer = [answer[0], answer[1], int(answer[2]), int(answer[3]), title.rstrip()]
                else:
                    answer = ['error', 'not a valid calendar date']
        if len(answer) == 2:
            if answer[0] == "show":
                answer = ["error", 'show']
        ...
        if answer[0] == "delete":
            if len(answer) >=1 and len(answer) <=2:
                answer = ['error', "delete DATE START_TIME"]
            if len(answer) == 3 and not is_calendar_date(answer[1]):
                    answer = ["error", 'not a valid calendar date']
            if len(answer) == 3:
                if not is_natural_number(answer[2]):
                    answer = ["error", 'not a valid event start time']
                else:
